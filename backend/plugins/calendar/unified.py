...
- Writes route to a single provider chosen by the account label.
- Label resolution goes through core.config.settings.ACCOUNT_PROVIDERS
  (runtime overrides land in MongoDB later via core.auth.account_labels).

Event objects returned from reads are stamped by each provider with the
account label they came from, so the LLM can pass that label straight
back into update_event / delete_event.
"""
...
class UnifiedCalendarClient:
    ...
    def __init__(self, providers: List[CalendarProvider]):
        ...
        # Keyed by provider.name for account-routed writes.
    ...
    def resolve_account(self, account: Optional[str]) -> CalendarProvider:
        """Map an account label ("personal" | "work") to its backing provider.

        - None + single provider: that provider.
        - None + multiple providers: raise — caller must supply an account.
        - Known label: routed via settings.ACCOUNT_PROVIDERS.
        - Unknown label: raise ValueError.
        """
        if account is None:
            if len(self._providers) == 1:
                return self._providers[0]
            raise ValueError(
                "Multiple calendar accounts are connected; pass account='personal' or 'work'."
            )

        mapping: Dict[str, str] = dict(settings.ACCOUNT_PROVIDERS or {})
        provider_name = mapping.get(account)
        if not provider_name:
            raise ValueError(
                f"Unknown account label '{account}'. Known labels: {sorted(mapping.keys())}"
            )
        provider = self._by_name.get(provider_name)
        if not provider:
            raise NeedsReauth(provider_name)
        return provider

    # ------------------------------------------------------------------
    # Reads — fan out, merge, dedup, sort
    # ------------------------------------------------------------------
    ...
    async def list_events(
        ...
        # Dedup by (account, id) so the same event id arriving from two providers
        # stays distinct. Account is 1:1 with provider at runtime.
    ...
    async def get_event(
        ...
        # No account hint — try each provider until one returns the event.
    ...
    # ------------------------------------------------------------------
    # Refresh — each provider manages its own token
    # ------------------------------------------------------------------
...
async def _try_provider(
    provider_name: str,
    scopes: List[str],
    build: Callable[[str, Optional[str]], CalendarProvider],
    account: Optional[str],
) -> Optional[CalendarProvider]:
    """Ensure scopes + build one provider. Returns None on missing/bad credentials."""
    try:
        token = await auth_manager.ensure_scopes(provider_name, scopes)
        return build(token.access_token, account)
...
async def build_unified_client() -> UnifiedCalendarClient:
    """
    Build the UnifiedCalendarClient for whichever OAuth providers are connected.

    Fail-safe: zero connected providers raises NeedsReauth("calendar");
    one provider works fine. Account labels come from settings.ACCOUNT_PROVIDERS.
    """
    ...
    # Reverse the label->provider mapping to get the first label for each provider.
    provider_to_label: Dict[str, str] = {}
    for label, provider_name in (settings.ACCOUNT_PROVIDERS or {}).items():
        provider_to_label.setdefault(provider_name, label)

    candidates = await asyncio.gather(
        _try_provider(
            "google", GOOGLE_CALENDAR_SCOPES,
            lambda t, a: GoogleProvider(create_google_client(t), account=a),
            provider_to_label.get("google"),
        ),
        _try_provider(
            "microsoft", OUTLOOK_CALENDAR_SCOPES,
            lambda t, a: OutlookProvider(create_outlook_client(t), account=a),
            provider_to_label.get("microsoft"),
        ),
    )
    providers = [p for p in candidates if p is not None]
...
# ---------------------------------------------------------------------------
# IntegrationManager factory glue
# ---------------------------------------------------------------------------
