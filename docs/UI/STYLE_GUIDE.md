...
- `TextLink` is for inline text actions that flow with surrounding copy (open docs, jump to a related surface). Underlined brand text, optional `external` affordance — not a pill and not a substitute for primary `Button` CTAs.
