...
- Built as a Stage-1-independent speaker embedding verifier, not an openWakeWord feature/logistic-regression verifier. Runtime math lives in `backend/core/voice/speaker_verifier.py` (`EnrolledSpeakerVerifier`): one process-shared immutable Sherpa extractor plus a session-scoped, atomically replaceable owner gallery scored by mean cosine.
- Wake Stage 2b (`SpeakerEmbeddingWakeVerifier`) is a thin `WakeVerifier` adapter over that shared verifier. The same instance scores barge-in candidates with `barge_in_speaker_threshold` (calibrate via `eval_wakeword.py --speaker-only`, including `--tts-echo` room-replay clips).
